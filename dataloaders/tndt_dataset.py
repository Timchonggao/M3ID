...
class TNDTDataset(data.Dataset):
    def __init__(self, mode='train', transform=None, return_size=False, fold=0, Balance=False):
        self.mode = mode
        self.transform = transform
        self.return_size = return_size

        root = '/data3/gaochong/dataset/TNDT/'
        
        
        trainvaltest = json.load(open(root + 'tndt-trainval-fold' + str(fold) + '.json', 'r')) # 加载json文件，划分好的训练集、验证集、测试集
        
        if mode == 'train':
            imgs, train_benign_num, train_malignant_num = make_dataset(root, trainvaltest['train'])
            self.train_benign_num = train_benign_num
            self.train_malignant_num = train_malignant_num
        elif mode == 'val':
            imgs, val_benign_num, val_malignant_num = make_validset(root, trainvaltest['val'], balance = Balance)
            self.val_benign_num = val_benign_num
            self.val_malignant_num = val_malignant_num
        elif mode == 'test':
    ...
    def __getitem__(self, item):
        ...
        if mask.max() != 0:
            mask = mask / mask.max() # FIXME 为啥要归一化？
