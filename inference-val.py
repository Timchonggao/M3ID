...
def main(args):
    ...
    for fold in range(5):
        if args.backbone == 'resnet18':
            ...
            pretrained_path = '/data/gaochong/project/Pswinnet/run/swinunet_lr0.001/fold{}/models/best_backbone.pth'.format(fold)
            backbone = swinunet_cis(pretrained=True, pretrained_path=pretrained_path)
            static_data_dir = '/data/gaochong/project/Pswinnet/static_info'
        ...
        if args.dataset == 'tn3k':
            valset = tn3k_dataset.TN3KDataset(mode='val', fold=fold, transform=composed_transforms_ts, return_size=False)
            valloader = DataLoader(valset, batch_size=1, shuffle=False, num_workers=2)
        elif args.dataset == 'tndt':
            valset = tndt_dataset.TNDTDataset(mode='val', fold=fold, transform=composed_transforms_ts, return_size=False)
            valloader = DataLoader(valset, batch_size=1, shuffle=False, num_workers=2)

        for sample_batched in tqdm(valloader):
            img = sample_batched['image'].cuda()
            label = sample_batched['label'].cuda()
            mask = sample_batched['mask'].cuda().float()

            if args.backbone in ['logonet', 'swinunet']:
        ...
        curacc = 0.5 * (acc_good / valset.val_benign_num + acc_bad / valset.val_malignant_num) # 加权正确率,差不多
        # curacc = (acc_good + acc_bad) / len(valset) # 正确率
        ...
        print(f'{args.dataset} val partition, fold {fold}', 'acc:'+accstr+' precision:'+presicionstr+' recall:'+recallstr+' f1:'+F1str+' auc:'+aucstr)

        if total_iou != 0:
            metrics_result = metrics.mean(len(valloader))
    ...
    csvFile = open(f"../../results/test-result/{args.dataset}-val.csv", "a+")
