...
def plot_probability_distribution(train_data, val_data, test_data, bin_width=0.05):
    """
...
def fuse_probabilities(prob_label0, prob_label1):
    pass
...
#     print("y_pred_train: ", y_pred_train)
    ...
    # 预测测试集
    y_pred_proba = catboost_model.predict_proba(X_test_normal)
    y_pred_proba_ = test_data[:, -3:-1]
    ...
    for i in range(y_pred_test.shape[0]):
        ...
        probability_ = y_pred_proba_[i]
        if probability_[1] < 0.4 and probability_[1] > 0.05 and probability[1]<0.4 and probability[1]>0.05:
            # pass
             probability_ = probability
            # probability_ = [(probability_[0] + probability[0])/2, (probability_[1] + probability[1])/2]

        pred = np.argmax(probability_)

        labels.append(y_test[i])
        preds.append(probability_[1])
    ...
    accuracy = 0.5 * (acc_good / 378 + acc_bad / 236) # 加权正确率,差不多

    plot_probability_distribution_with_misclassification_rate(y_test, y_pred_proba[:, 1])
    Acc.append(accuracy)
    Presicion.append(presicion)
    Recall.append(recall)
    F1.append(f1)
    Auc.append(auc)

    print('test fold %d acc: %.4f, auc: %.4f, f1: %.4f, presicion: %.4f, recall: %.4f \n' % \
          (fold, accuracy, auc, f1, presicion, recall))  
    # breakpoint()
