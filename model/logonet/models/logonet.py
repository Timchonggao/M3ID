...
class TAD_block(nn.Module):
    ...
    def forward(self, x):
        ...
        proj_query = proj_query - proj_query.mean(1).unsqueeze(1)
        proj_key = self.key_conv(x).view(B, -1, H * W)  # B * C * N
        proj_key = proj_key - proj_key.mean(2).unsqueeze(2)
...
class logonet(nn.Module):
    ...
    def forward(self, x): # input size is torch.Size([16, 3, 224, 224])
        # breakpoint()
        x = self.conv1(x) # torch.Size([16, 64, 112, 112])
        x = self.bn1(x)
        x = self.relu(x)
        x1 = self.maxpool(x) # torch.Size([16, 64, 56, 56])

        x, tissue1 = self.tad1(x1) # x shape is torch.Size([16, 64, 56, 56]), tissue1 shape is torch.Size([16, 64, 56, 56])
        x2 = self.layer1(x) # torch.Size([16, 64, 56, 56])
        x, tissue2 = self.tad2(x2) # x shape is torch.Size([16, 64, 56, 56]), tissue2 shape is torch.Size([16, 64, 56, 56])
        x3 = self.layer2(x) # torch.Size([16, 128, 28, 28])
        x, tissue3 = self.tad3(x3) # x shape is torch.Size([16, 128, 28, 28]), tissue3 shape is torch.Size([16, 128, 28, 28])
        x4 = self.layer3(x) # torch.Size([16, 256, 14, 14])
        x, tissue4 = self.tad4(x4) # x shape is torch.Size([16, 256, 14, 14]), tissue4 shape is torch.Size([16, 256, 14, 14])
        x5 = self.layer4(x) # torch.Size([16, 512, 7, 7])

        x = self.avgpool(x5) # torch.Size([16, 512, 1, 1])
        x = x.view(x.size(0), -1) # torch.Size([16, 512])
        x = self.fc(x) # torch.Size([16, 2])

        y = self.up4(x5, tissue4) # torch.Size([16, 256, 14, 14])
        y = self.up3(y, tissue3) # torch.Size([16, 128, 28, 28])
        y = self.up2(y, tissue2) # torch.Size([16, 64, 56, 56])
        y = self.up1(y, tissue1) # torch.Size([16, 64, 56, 56])
        y = self.outconv(y) # torch.Size([16, 1, 56, 56])

        return x, y

# FIXME 这种加载权重的方式比较好用,对于两个具有相同层的网络，但是似乎没有检查layer参数的shape一致
